function [Est, P, MD_2] = EKF_bias(Est, P, B_sat, B_eci, xsat_eci, Kalman_parameters,h, index)
...
B_body_norm = B_model/ norm(B_model);
...
[A, Est] = motion_integration_bias(Est, xsat_eci, B_model, index);
...
W_b = [0     -B_body_norm(3)  B_body_norm(2);
       B_body_norm(3)  0     -B_body_norm(1);
      -B_body_norm(2)   B_body_norm(1)    0];       % [B0 x]
...
G = [zeros(3,9);[inv(J) zeros(3,6)];[zeros(3,3) eye(3,3) zeros(3,3)];[zeros(3,6) eye(3,3)]];
...
% If using the bias correction for the filter, remove the '%' below
...
v_k = (B_sat/norm(B_model)-B_body); % Innovation term
MD_2 = v_k'*inv(S_k)*v_k; % Mahalanobis Distance
disp(MD_2)
end
