function  [satrec, xsat_ecf, vsat_ecf, xsat_eci, vsat_eci, gst]=spg4_ecf(satrec,tsince);
...
xsat_ecf(1,1)= xsat_eci(1)*CGAST+xsat_eci(2)*SGAST;
xsat_ecf(2,1)=-xsat_eci(1)*SGAST+xsat_eci(2)*CGAST;
xsat_ecf(3,1)= xsat_eci(3);
%Apply rotation to convert velocity vector from ECI to ECEF coordinates
OMEGAE = 7.29211586D-5;  %Earth rotation rate in rad/s
vsat_ecf(1,1)= vsat_eci(1)*CGAST+vsat_eci(2)*SGAST+OMEGAE*xsat_ecf(2);
vsat_ecf(2,1)=-vsat_eci(1)*SGAST+vsat_eci(2)*CGAST-OMEGAE*xsat_ecf(1);
vsat_ecf(3,1)= vsat_eci(3);
return